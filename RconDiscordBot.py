...
if os.path.exists('RconDiscordBot_config.json'):
    with open('RconDiscordBot_config.json') as file:
        config = json.load(file)
else:
    open('RconDiscordBot_config.json', 'w')
    with open('RconDiscordBot_config.json', 'w') as file:
        default = {'TOKEN': '',
                   'PREFIX': '--',
                   'IP': '127.0.0.1',
                   'rcon_pw': '',
    ...
    print("No config detected. Please edit RconDiscordBot_config.json and run this program again.")
    exit()
